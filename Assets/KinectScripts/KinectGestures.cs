...
//							}
            ...
            case Gestures.Bow:
                //Vector3 vectorBow1 = jointsPos[rightHandIndex] -jointsPos[shoulderCenterIndex];
                //Vector3 vectorBow2 = jointsPos[leftHandIndex] - jointsPos[shoulderCenterIndex];
                Vector3 vectorBow = jointsPos[shoulderCenterIndex];
                //Vector3 rightHandTest = jointsPos[rightHandIndex];
                //textLog.GetComponent<Text>().text = "righthand x:"+rightHandTest.x+" y:"+rightHandTest.y+" z:"+rightHandTest.z;
                float distBow = vectorBow.magnitude;
                //Debug.Log(string.Format("vecterbow:x{0},y{1},z{2}.vecterbow1:x{3},y{4},z{5}.vectorbow2:x{6},y{7},z{8}", vectorBow.x,vectorBow.y,vectorBow.z, vectorBow1.x, vectorBow1.y, vectorBow1.z, vectorBow2.x, vectorBow2.y, vectorBow2.z));
                //				Debug.Log(string.Format("{0}. Dist: {1:F1}, Tag: {2:F1}, Diff: {3:F1}", gestureData.state,
                //				                        distWheel, gestureData.tagFloat, Mathf.Abs(distWheel - gestureData.tagFloat)));
                ...
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] //&&
                           //jointsPos[leftHandIndex].y <= jointsPos[hipCenterIndex].y /*&& jointsPos[leftHandIndex].y <= gestureTop */&&
                           //jointsPos[rightHandIndex].y <= jointsPos[hipCenterIndex].y/* && jointsPos[rightHandIndex].y <= gestureTop &&
                           //distBow >= 0.3f && distBow < 0.7f)*/
                                )
                        {
                            SetGestureJoint(ref gestureData, timestamp, shoulderCenterIndex, jointsPos[shoulderCenterIndex]);
                            gestureData.tagVector = Vector3.up ;
                            gestureData.tagFloat = distBow;
                            gestureData.progress = 0.3f;
                        }
                        break;
                    ...
                    case 1:  // gesture phase 2 = zooming
                        if ((timestamp - gestureData.timestamp) < 0.5f)
                        {
                            float angle = Vector3.Angle(gestureData.tagVector, new Vector3(0,vectorBow.y,vectorBow.z).normalized) * Mathf.Sign(vectorBow.z);
                            bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] /*&&
                                 jointsPos[leftHandIndex].y <= jointsPos[hipCenterIndex].y /* && jointsPos[leftHandIndex].y <= gestureTop *///&&
                                //jointsPos[rightHandIndex].y <= jointsPos[hipCenterIndex].y /*&& jointsPos[rightHandIndex].y <= gestureTop &&
                                //distBow >= 0.3f && distBow < 0.7f &&
                                //Mathf.Abs(distBow - gestureData.tagFloat) < 0.1f*/
                                ;
                            ...
                            if (isInPose)
                            {
                                ...
                                gestureData.tagFloat = distBow;
                                gestureData.progress = 0.7f;
                                gestureData.vectorOut = vectorBow;
                            }
                            //							else
                            //							{
                            //								// cancel the gesture
